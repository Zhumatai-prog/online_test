...
def update_student_view(request,pk):
    ...
    mydict={'student':student, 'userForm':userForm,'studentForm':studentForm}
    if request.method=='POST':
...
#class AdminAddQuestion(CreateView):
#    model = Question
...
def admin_check_marks_view(request,pk):
    ...
    questions = Question.objects.all().filter(course=course).count()
    results = Result.objects.all().filter(exam=course).filter(student=student)
    return render(request,'quiz/admin_check_marks.html',{'results':results, 'questions': questions})
